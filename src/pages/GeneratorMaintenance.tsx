...
import { Plus, Eye, Pencil, Trash2, Search, Zap } from 'lucide-react';
...
export default function GeneratorMaintenance() {
  ...
  const { data: checklists, isLoading } = useQuery({
    queryKey: ['generator-maintenance-checklists', statusFilter],
    queryFn: async () => {
      let query = supabase
  ...
  const filteredChecklists = checklists?.filter((checklist: any) => {
    ...
    return (
      checklist.generators?.equipment_name?.toLowerCase().includes(searchLower) ||
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        <div className="relative flex-1">
          ...
          <Input
            placeholder="Search by generator or performed by..."
            value={searchTerm}
                  ...
                  <TableCell className="text-right">
                    <div className="flex justify-end gap-2">
                      <Button
                        variant="ghost"
                        size="icon"
                        onClick={() => navigate(`/generator-maintenance/${checklist.id}`)}
                      >
                      ...
                      <Button
                        variant="ghost"
                        size="icon"
                        onClick={() => navigate(`/generator-maintenance/${checklist.id}/edit`)}
                      >

...
import { Plus, Eye, Pencil, Trash2, Search, Car } from 'lucide-react';
...
export default function VehicleMaintenance() {
  ...
  const { data: checklists, isLoading } = useQuery({
    queryKey: ['vehicle-maintenance-checklists', statusFilter],
    queryFn: async () => {
      let query = supabase
  ...
  const filteredChecklists = checklists?.filter((checklist: any) => {
    ...
    return (
      checklist.vehicles?.plate_no?.toLowerCase().includes(searchLower) ||
      checklist.vehicles?.make_brand?.toLowerCase().includes(searchLower) ||
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        <div className="relative flex-1">
          ...
          <Input
            placeholder="Search by vehicle or performed by..."
            value={searchTerm}
            ...
            {isLoading ? (
              ...
              filteredChecklists.map((checklist: any) => (
                <TableRow key={checklist.id}>
                  <TableCell>
                    {checklist.vehicles?.plate_no || 'N/A'} - {checklist.vehicles?.make_brand || ''}
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell className="text-right">
                    <div className="flex justify-end gap-2">
                      <Button
                        variant="ghost"
                        size="icon"
                        onClick={() => navigate(`/vehicle-maintenance/${checklist.id}`)}
                      >
                      ...
                      <Button
                        variant="ghost"
                        size="icon"
                        onClick={() => navigate(`/vehicle-maintenance/${checklist.id}/edit`)}
                      >
